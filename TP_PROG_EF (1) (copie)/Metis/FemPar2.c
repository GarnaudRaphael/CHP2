...
int sommeDoubleTableau(int tableau1[],int tableau2[], int tailleTableau)
{
  int sommeTableau;
  for (int i=0; i<tailleTableau; i++)
{
   sommeTableau=tableau1[i]*tableau2[i]/Shared[i]+sommeTableau;
}
return sommeTableau;
}
...
float InnerProduct(float *A1, float *A2, float *B1, float *B2)
{
   ...
   //IP = A1*B1+sommeDoubleTableau(A2,B2,IBNodes);
   ...
   MPI_Allreduce(&IP,&IPTotal,1,MPI_FLOAT,MPI_SUM,MPI_COMM_WORLD);//On mets les IP de tous les procs dans IPTotal ; c'est juste un nombre donc taille 1
   return(IPTotal);
}
...
void Update(float *A)
{
   //int I, J;
   MPI_Status Status;
   //J'envoie mon assemblage à mes voisins si j'ai des noeuds en commun
   for(int i=0;i<ProcNo;i++)
   {
     ...
     if(Common[i]>0)
     {
         MPI_Send(&Buf,Common[i],MPI_FLOAT,i,10,MPI_COMM_WORLD);//Buf[0] on envoie le premier élément du tableau
     }

  }
  //Je reçois les combinaisons des éléments voisins pour mes noeuds en commum
  for(int i=0; i<ProcNo ; i++)
  {
    if(Common[i]>0)
    {
      MPI_Recv(&Buf,Common[i],MPI_FLOAT,i,10,MPI_COMM_WORLD,&Status);
    }
    for(int j=0;j<Common[i];j++)
    {
